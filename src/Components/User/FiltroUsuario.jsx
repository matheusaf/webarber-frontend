...
const FiltroUsuario = () => {
	...
	const fetchUsers = useCallback(async () => {
		...
		try {
			let req = await fetch(
				`${url}/filtro?${query.tipoUsuario.value}=${query.query.value}`,
				{
					method: "GET",
	...
	const tableRow = (obj) => {
		return (
			<tr key={`row-${obj.id}`}>
				<td data-testid={`valor-${obj.nome}`} key={`valor-${obj.nome}`}>
					${obj.nome}
				</td>
				<td
					data-testid={`valor-${obj.sobrenome}`}
					key={`valor-${obj.sobrenome}`}
				>
					${obj.sobrenome}
				</td>
				<td data-testid={`valor-${obj.email}`} key={`valor-${obj.email}`}>
					${obj.email}
				</td>
				<td data-testid={`valor-${obj.ativo}`} key={`valor-${obj.ativo}`}>
					${obj.ativo}
				</td>
				<td data-testid={`valor-${obj.CNPJ}`} key={`valor-${obj.CNPJ}`}>
					${obj.CNPJ}
				</td>
				<td data-testid={`valor-${obj.CPF}`} key={`valor-${obj.CPF}`}>
					${obj.CPF}
				</td>
				<td data-testid={`valor-${obj.idTipo}`} key={`valor-${obj.idTipo}`}>
					${obj.idTipo}
				</td>
			</tr>
		);
		...
		<>
			...
			<div>
				...
				{Object.keys(query).map((field) => (
					<Input
						elementType={query[`${field}`].elementType}
						elementConfig={query[`${field}`].elementConfig}
